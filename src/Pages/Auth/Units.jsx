...
import { adminUnitAPI } from '../../api/api'
...
const COLUMN_DEFS = [
  ...
  { key: 'factor', label: { en: 'Factor', kh: 'កត្តា' } },
  { key: 'active', label: { en: 'Active', kh: 'ដំណើរការ' } },
]
const DEFAULT_COLS = ['description', 'nameKh', 'factor', 'active']

const T = {
  subtitle: {
    en: 'Define units such as kg, g, L, ml and piece.',
    kh: 'កំណត់ឯកតាដូចជា គីឡូ ក្រាម លីត្រ មីលីលីត្រ និងដុំ។',
  },
  newBtn: { en: '+ New Unit', kh: '+ ឯកតាថ្មី' },
  // filter bar
  ...
  searchPlaceholder: {
    en: 'Type to search units…',
    kh: 'បញ្ចូលដើម្បីស្វែងរកឯកតា…',
  },
  statusLabel: { en: 'Status', kh: 'ស្ថានភាព' },
  allUnits: { en: 'All Units', kh: 'ឯកតាទាំងអស់' },
  ...
  noResults: { en: 'No units match your filters.', kh: 'រកមិនឃើញឯកតាវាស់តាមតម្រូវការ។' },
  // columns
  code: { en: 'Code', kh: 'កូដ' },
  ...
  // form
  newTitle: { en: 'Create Unit', kh: 'បង្កើតឯកតា' },
  editTitle: { en: 'Edit Unit', kh: 'កែឯកតា' },
  autoGenTitle: {
    en: 'Leave blank and the backend generates the next free UN code — or type your own',
    kh: 'ទុកចំហចង់បាន — ម៉ាស៊ីនមេនឹងបង្កើតកូដ UN បន្ទាប់ដោយស្វ័យប្រវត្តិ — ឬបញ្ចូលដោយខ្លួនឯង',
  },
  descPlaceholder: { en: 'e.g. Kilogram (kg)', kh: 'ឧ. គីឡូក្រាម' },
  secondLangPlaceholder: { en: 'ឧ. គីឡូក្រាម', kh: 'ឧ. គីឡូក្រាម' },
  factorHint: {
    en: 'Conversion factor relative to the base unit, e.g. kg = 1000, g = 1',
    kh: 'កត្តាបំលែងទាក់ទងនឹងឯកតាមូលដ្ឋាន ឧ. គីឡូ = ១០០០ ក្រាម = ១',
  },
  edit: { en: 'Edit', kh: 'កែ' },
  delete: { en: 'Delete', kh: 'លុប' },
  create: { en: 'Create', kh: 'បង្កើត' },
  save: { en: 'Save', kh: 'រក្សាទុក' },
  deleteConfirm: { en: 'Delete this unit?', kh: 'លុបឯកតានេះ?' },
  empty: { en: 'Nothing here yet — create your first unit.', kh: 'មិនទាន់មានទិន្នន័យទេ — បង្កើតជួរដេកដំបូងរបស់អ្នក។' },
  loadFailed: { en: 'Could not load units from the server.', kh: 'មិនអាចផ្ទុកឯកតាពីម៉ាស៊ីនមេបានទេ។' },
}
...
const EMPTY_FORM = { id: null, code: '', description: '', nameKh: '', factor: '', active: true }
...
export const Units = () => {
  ...
  // Fetch the unit list; `refresh` is also used after create/update/delete.
  ...
  const openEdit = (item) => {
    setForm({
      ...
      nameKh: item.nameKh || '',
      // keep the raw backend value; '' renders as an empty input
      factor: item.factor ?? '',
  ...
  // UN-#### automatically; a typed code must be unique (409 otherwise).
  // Factor is parsed to a number — blank clears it on the backend.
  const saveForm = async (e) => {
    e.preventDefault()
    const factorRaw = String(form.factor).trim()
    const payload = {
      code: form.code.trim() || undefined,
      description: form.description.trim(),
      nameKh: form.nameKh.trim() || undefined,
      factor: factorRaw === '' || isNaN(Number(factorRaw)) ? null : Number(factorRaw),
      active: form.active,
    }
    try {
      if (form.id != null) await adminUnitAPI.update(form.id, payload)
      else await adminUnitAPI.create(payload)
      setFormOpen(false)
      refresh()
    } catch (err) {
  ...
  const removeItem = async (item) => {
    ...
    try {
      await adminUnitAPI.delete(item.id)
      refresh()
    } catch (err) {
      window.alert(err.message)
    }
  ...
  const renderCell = (item, key) => {
    switch (key) {
      ...
      case 'factor':
        return <span className="font-mono text-slate-300">{item.factor ?? '—'}</span>
      case 'active':
    ...
    >
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-700/60 bg-slate-900/80 shadow-xl shadow-black/20">
        ...
        <div className="flex flex-col gap-3 border-b border-slate-700/60 p-4 lg:flex-row lg:items-center">
          ...
          >
            <option value="all">{t('allUnits', lang)}</option>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <Field label={t('code', lang)}>
              <div className="flex items-center gap-2">
                <TextInput value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} placeholder="UN-0001" />
                {!form.id && !form.code && (
            ...
            {/* Factor — conversion relative to the base unit (optional) */}
            <Field label={t('factor', lang)}>
              <TextInput
                type="number"
                step="any"
                value={form.factor}
              ...
              />
              <span className="mt-1 block text-xs text-slate-500">{t('factorHint', lang)}</span>
            </Field>
          </div>
