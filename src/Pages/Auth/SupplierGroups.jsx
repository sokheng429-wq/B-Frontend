...
import { adminSupplierGroupAPI } from '../../api/api'
...
const T = {
  subtitle: {
    en: 'Organize suppliers into regional or category groups.',
    kh: 'រៀបចំអ្នកផ្គត់ផ្គង់ជាក្រុមតាមតំបន់ ឬប្រភេទ។',
  },
  newBtn: { en: '+ New Supplier Group', kh: '+ ក្រុមអ្នកផ្គត់ផ្គង់ថ្មី' },
  // filter bar
  ...
  searchPlaceholder: {
    en: 'Type to search supplier groups…',
    kh: 'បញ្ចូលដើម្បីស្វែងរកក្រុមអ្នកផ្គត់ផ្គង់…',
  },
  statusLabel: { en: 'Status', kh: 'ស្ថានភាព' },
  allSupplierGroups: { en: 'All Supplier Groups', kh: 'ក្រុមអ្នកផ្គត់ផ្គង់ទាំងអស់' },
  ...
  noResults: { en: 'No supplier groups match your filters.', kh: 'រកមិនឃើញក្រុមអ្នកផ្គត់ផ្គង់តាមតម្រូវការ។' },
  // columns
  code: { en: 'Code', kh: 'កូដ' },
  ...
  // form
  newTitle: { en: 'Create Supplier Group', kh: 'បង្កើតក្រុមអ្នកផ្គត់ផ្គង់' },
  editTitle: { en: 'Edit Supplier Group', kh: 'កែក្រុមអ្នកផ្គត់ផ្គង់' },
  autoGenTitle: {
    en: 'Leave blank and the backend generates the next free SG code — or type your own',
    kh: 'ទុកចំហចង់បាន — ម៉ាស៊ីនមេនឹងបង្កើតកូដ SG បន្ទាប់ដោយស្វ័យប្រវត្តិ — ឬបញ្ចូលដោយខ្លួនឯង',
  },
  descPlaceholder: { en: 'e.g. Siem Reap Farms', kh: 'ឧ. កសិដ្ឋានសៀមរាប' },
  secondLangPlaceholder: { en: 'ឧ. កសិដ្ឋានសៀមរាប', kh: 'ឧ. កសិដ្ឋានសៀមរាប' },
  edit: { en: 'Edit', kh: 'កែ' },
  delete: { en: 'Delete', kh: 'លុប' },
  create: { en: 'Create', kh: 'បង្កើត' },
  save: { en: 'Save', kh: 'រក្សាទុក' },
  deleteConfirm: { en: 'Delete this supplier group?', kh: 'លុបក្រុមអ្នកផ្គត់ផ្គង់នេះ?' },
  empty: { en: 'Nothing here yet — create your first supplier group.', kh: 'មិនទាន់មានទិន្នន័យទេ — បង្កើតជួរដេកដំបូងរបស់អ្នក។' },
  loadFailed: { en: 'Could not load supplier groups from the server.', kh: 'មិនអាចផ្ទុកក្រុមអ្នកផ្គត់ផ្គង់ពីម៉ាស៊ីនមេបានទេ។' },
}
...
export const SupplierGroups = () => {
  ...
  // Fetch the supplier group list; `refresh` is also used after create/update/delete.
  ...
  const saveForm = async (e) => {
    ...
    try {
      if (form.id != null) await adminSupplierGroupAPI.update(form.id, payload)
      else await adminSupplierGroupAPI.create(payload)
      setFormOpen(false)
      refresh()
    } catch (err) {
  ...
  const removeItem = async (item) => {
    ...
    try {
      await adminSupplierGroupAPI.delete(item.id)
      refresh()
    } catch (err) {
      window.alert(err.message)
    }
    ...
    >
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-700/60 bg-slate-900/80 shadow-xl shadow-black/20">
        ...
        <div className="flex flex-col gap-3 border-b border-slate-700/60 p-4 lg:flex-row lg:items-center">
          ...
          >
            <option value="all">{t('allSupplierGroups', lang)}</option>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <Field label={t('code', lang)}>
              <div className="flex items-center gap-2">
                <TextInput value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} placeholder="SG-0001" />
                {!form.id && !form.code && (
