...
const T = {
  subtitle: {
    en: 'Manage the product brands shown across the shop.',
    kh: 'គ្រប់គ្រងម៉ាកទំនិញដែលបង្ហាញក្នុងហាងទាំងមូល។',
  },
  newBtn: { en: '+ New Brand', kh: '+ ម៉ាកថ្មី' },
  // filter bar
  ...
  searchPlaceholder: {
    en: 'Type to search brands…',
    kh: 'បញ្ចូលដើម្បីស្វែងរកម៉ាក…',
  },
  ...
  noResults: { en: 'No brands match your filters.', kh: 'រកមិនឃើញម៉ាកទំនិញតាមតម្រូវការ។' },
  // columns
  code: { en: 'Code', kh: 'កូដ' },
  ...
  // form
  newTitle: { en: 'Create Brand', kh: 'បង្កើតម៉ាក' },
  editTitle: { en: 'Edit Brand', kh: 'កែម៉ាក' },
  autoGenTitle: {
    en: 'Leave blank and the backend generates the next free BR code — or type your own',
    kh: 'ទុកចំហចង់បាន — ម៉ាស៊ីនមេនឹងបង្កើតកូដ BR បន្ទាប់ដោយស្វ័យប្រវត្តិ — ឬបញ្ចូលដោយខ្លួនឯង',
  },
  descPlaceholder: { en: 'e.g. Angkor Milk', kh: 'ឧ. អង្គរមីល្ក៍' },
  ...
  deleteConfirm: { en: 'Delete this brand?', kh: 'លុបម៉ាកនេះ?' },
  empty: { en: 'Nothing here yet — create your first brand.', kh: 'មិនទាន់មានទិន្នន័យទេ — បង្កើតជួរដេកដំបូងរបស់អ្នក។' },
  loadFailed: { en: 'Could not load brands from the server.', kh: 'មិនអាចផ្ទុកម៉ាកពីម៉ាស៊ីនមេបានទេ។' },
}
...
export const Brands = () => {
  ...
  // Fetch the brand list; `refresh` is also used after create/update/delete.
  ...
  const saveForm = async (e) => {
    ...
    try {
      if (form.id != null) await adminBrandAPI.update(form.id, payload)
      else await adminBrandAPI.create(payload)
      setFormOpen(false)
      refresh()
    } catch (err) {
  ...
  const removeItem = async (item) => {
    ...
    try {
      await adminBrandAPI.delete(item.id)
      refresh()
    } catch (err) {
      window.alert(err.message)
    }
    ...
    >
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-700/60 bg-slate-900/80 shadow-xl shadow-black/20">
        ...
        <div className="flex flex-col gap-3 border-b border-slate-700/60 p-4 lg:flex-row lg:items-center">
          ...
          >
            <option value="all">{t('allBrands', lang)}</option>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <Field label={t('code', lang)}>
              <div className="flex items-center gap-2">
                <TextInput value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} placeholder="BR-0001" />
                {!form.id && !form.code && (
